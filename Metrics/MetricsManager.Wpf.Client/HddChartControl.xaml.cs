        ...
        private void UpdateOnСlick(object sender, RoutedEventArgs e)
        {
            Task.Run(() =>
            {
                while (true)
                {
                    ...
                    try
                    {
                        HddAllMetricsResponse metricsResponse = _metricsManagerClient.GetHddMetricsFromAgentAsync(request).Result;

                        HddMetric[] metrics = metricsResponse.Metrics.ToArray();
                        ...
                        Dispatcher.Invoke(() =>
                        {
                            if (metricsResponse.Metrics.Count > 0)
                            {
                                ...
                                PersentTextDesciption = $"За последние {del.TotalSeconds} секунд % времени использования диска";
