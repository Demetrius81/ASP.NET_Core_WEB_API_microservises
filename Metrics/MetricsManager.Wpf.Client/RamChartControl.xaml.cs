        ...
        private void UpdateOnСlick(object sender, RoutedEventArgs e)
        {
            Task.Run(() =>
            {
                while (true)
                {
                    ...
                    try
                    {
                        RamAllMetricsResponse metricsResponse = _metricsManagerClient.GetRamMetricsFromAgentAsync(request).Result;

                        RamMetric[] metrics = metricsResponse.Metrics.ToArray();
                        ...
                        Dispatcher.Invoke(() =>
                        {
                            if (metricsResponse.Metrics.Count > 0)
                            {
                                ...
                                PersentTextDesciption = $"За последние {del.TotalSeconds} секунд объем доступной памяти";
